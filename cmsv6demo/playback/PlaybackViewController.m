...
@interface PlaybackViewController ()<RMDateSelectionViewControllerDelegate,TempSearchFoundtionDelegate,UITableViewDelegate,UITableViewDataSource>
{
//    NSInteger selectTerminal;
...
//        return;
//    }
//    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"terminal" message:@"" preferredStyle:UIAlertControllerStyleActionSheet];
//
//    for (int i = 0;  i < _deviceList.count; i++) {
//        VehicleInfoModel *vmodel = _deviceList[i];
//          EquipmentInfoModel *emodel = vmodel.dl[0];
//        NSString *title = [NSString stringWithFormat:@"%@",vmodel.nm];
//        UIAlertAction *action = [UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//            VehicleInfoModel *vmodel = _deviceList[i];
//            EquipmentInfoModel *emodel = vmodel.dl[0];
//            NSString *title1 = emodel.id2;
//            [_terminal setTitle:title1 forState:UIControlStateNormal];
//            selectTerminal = i;
//        }];
...
- (IBAction)searchBtn:(id)sender {
    if (_currentVehicle != nil) {
        ...
        TempSearchFoundtion * tsf = [[TempSearchFoundtion alloc]init];
        tsf.delegate = self;

    EquipmentInfoModel *model1 = _currentVehicle.dl[0];
     NSString *deviceId  = model1.id2;
        [tsf onsearch:deviceId location:(int)_location.selectedSegmentIndex channel:(int)selectChannel type:(int)_videoType.selectedSegmentIndex-1 date:queryDate beginTime:nBeginTime endTime:nEndTime];
    }
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   if([model checkIsMultChn])
   {
       ...
       for (NSInteger k = 0; k < chn; k++) {
           if ( (chnMask >> k) & 0x01 ) {
               UIAlertAction *action = [UIAlertAction actionWithTitle:[NSString stringWithFormat:@"CH%zd",k+1] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                   model.selectChn = k;
